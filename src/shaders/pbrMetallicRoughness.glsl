...
float microfacetDistribution(float alphaRoughness, float NdotH)
{
 ...
 vec4 reflectionAttenuation(vec3 normal, vec3 view, vec3 lightColor, vec3 lightDirection, vec4 albedo, float metallicFactor, float roughnessFactor) {
	...
	vec3 color = NdotL * lightColor * specContrib;
