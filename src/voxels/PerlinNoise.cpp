...
uint8_t PerlinNoise::defaultPermutation[512] = {
...
PerlinNoise::PerlinNoise() : _perm(defaultPermutation) {}

PerlinNoise::PerlinNoise(unsigned int seed) : _perm(new uint8_t[512]) {
	std::iota(_perm, _perm + 256, 0);
	std::shuffle(_perm, _perm + 256, std::mt19937(seed));
	std::copy(_perm, _perm + 256, _perm + 256);
}

PerlinNoise::PerlinNoise(const PerlinNoise& P) : _perm(new uint8_t[512]) {
	std::copy(P._perm, P._perm + 512, _perm);
}

PerlinNoise& PerlinNoise::operator=(const PerlinNoise& P) {
	delete[] _perm;
	_perm = new uint8_t[512];
	std::copy(P._perm, P._perm + 512, _perm);
	return *this;
}

PerlinNoise::~PerlinNoise() {
	if(_perm != defaultPermutation)
		delete[] _perm;
}

double PerlinNoise::noise(double x, double y, double z) const {
	uint8_t X = static_cast<uint8_t>(std::floor(x)) & 255, Y = static_cast<uint8_t>(std::floor(y)) & 255, Z = static_cast<uint8_t>(std::floor(z)) & 255;
	...
	return lerp(w, lerp(v, lerp(u, grad(_perm[AA], x, y, z), grad(_perm[BA], x - 1, y, z)), lerp(u, grad(_perm[AB], x, y - 1, z), grad(_perm[BB], x - 1, y - 1, z))),
