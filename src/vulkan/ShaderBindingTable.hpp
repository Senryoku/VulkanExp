...
struct ShaderBindingTable {
	...
	void create(const Device& device, const std::array<uint32_t, 4> entriesCount, VkPipeline pipeline) {
		...
		if(!buffer) {
			...
			memory.allocate(device, buffer,
							VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		}
