...
class CommandBuffer : public HandleWrapper<VkCommandBuffer> {
  public:
    ...
    void bind(const std::array<VkBuffer, Count> buffers) const {
        ...
        vkCmdBindVertexBuffers(_handle, 0, buffers.size(), buffers.data(), offsets);
    }
};
...
class CommandBuffers {
  public:
    ...
    const CommandBuffer& operator[](size_t idx) const {
        return getBuffers()[idx];
    }
    const std::vector<CommandBuffer>& getBuffers() const {
        return _buffers;
    }
    const std::vector<VkCommandBuffer>& getBuffersHandles() const {
        return _bufferHandles;
    }
